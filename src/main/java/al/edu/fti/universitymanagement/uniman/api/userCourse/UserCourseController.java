...
public class UserCourseController extends BaseController<UserCourseDto, UserCourseEntity> {
    ...
    @Override
    public ResponseEntity<List<UserCourseDto>> filter(@RequestBody FilterWrap filterWrap) {
        return ResponseEntity.ok(baseService.findAll(filterWrap, UserCourseEntity.class));
